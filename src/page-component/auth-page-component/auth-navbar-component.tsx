...
const AuthNavbarComponent = () => {
  ...
  return (
    <Box w='full' zIndex={999}>
      <Container maxW='container.lg'>
        <Flex align='center' justify='space-between' h='10vh'>
          <HStack>
						<IconButton aria-label='menu' icon={<BiMenuAltLeft />} colorScheme={'teal'} variant={'solid'} display={{ base: 'flex', md: 'none' }} onClick={toggleMenu}/>
						<Link href={'/'}>
            ...
            {navigation[1].links.map(nav => (
              <Link key={nav.label} href={nav.route}>
                <Box display={{base: 'none', md: 'flex'}} color='facebook.200' _hover={{textDecoration: 'underline', color: linkHover}} as='a'>
